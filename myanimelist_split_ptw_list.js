...
function changeGraph(c, pt) {
  ...
  if (c !== 0) {
    ...
    $("div.anime>.stats-graph.mt8>.plan_to_watch").css(
      "width",
      newPtwBlockLength + "px"
    ); //reduces the size of the graph
...
var returnValue = (function () {
  ...
  $.get(
    "https://myanimelist.net/animelist/" + name + "?airing_status=3",
    function (data) {
      if (data.indexOf("list-container") === -1) {
        //this is a classic list, pull other page
        $.get(
          "https://myanimelist.net/animelist/" + name + "?status=6&tag=",
          function (classicData) {
            count = (classicData.match(/not yet aired/gi) || []).length; //find number of entries
            classicData = classicData + " " + externalVar;
            PTW = $(".di-ib.fl-l.lh10.circle.anime.plan_to_watch")
              .next()
              .text(); //get number of entries on PTW according to MAL
            if (PTW.indexOf(",") !== -1) {
              //remove commas
              PTW = PTW.replace(",", "");
            }
            if (PTW === "0") {
              //dont change it if there isnt anything on PTW
              count = 0;
            } else {
              $(
                "<style>.stats-status { width: 180px !important; }</style>"
              ).appendTo("head"); //change width to allow text to fit
              PTW = PTW - count;
              $(".di-ib.fl-l.lh10.circle.anime.plan_to_watch")
                .next()
                .html(PTW + "(+" + count + ")"); //change text in stats
            }
            changeGraph(count, PTW);
          }
        );
      } else {
        //(pageData.contains("list_surround"))
        //this is a modern list
        count = (data.match(/anime_title&quot;:&quot;/g) || []).length;
        data = data + " " + externalVar;
        var PTW = $(".di-ib.fl-l.lh10.circle.anime.plan_to_watch")
          .next()
          .text(); //find number of entries
        if (PTW.indexOf(",") !== -1) {
          //remove commas
          PTW = PTW.replace(",", "");
        }
