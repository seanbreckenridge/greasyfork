...
(function () {
  ...
  if (location.href.indexOf("/profile/") !== -1) {
    //if were on the profile page
    var planToWatch = $("div.anime")
      .find("ul.stats-status.fl-l")
      .children(":last-child");
    $(planToWatch).removeClass("mb12");
    $(planToWatch).addClass("mb8");
    $(planToWatch).after(
      '<li class="clearfix mb4" style="margin-left: 25px;">┗ <a href="https://myanimelist.net/animelist/' +
        $("h1").first().find("span").text().trim().slice(0, -10) +
        '?order=-16&status=6&openuserscript=true">Open Aired Entries</a></li>'
    );
  } else {
    //else if were on the page which this opens pages from
    var openedPages = 0;
